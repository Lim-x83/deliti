# 🍽️ Deliti - Enterprise Food Delivery Platform

> **A sophisticated full-stack mobile application solution built with cutting-edge Flutter architecture and robust PHP backend services.**

## 🏗️ Architectural Overview

Deliti represents a comprehensive food delivery ecosystem engineered with modern development practices and scalable infrastructure design.

### 🎯 Core Technology Stack
- **Frontend Architecture:** Flutter 3.0+ with Dart Null Safety
- **Backend Services:** PHP 8.2+ with PDO Database Abstraction
- **Data Persistence:** MySQL/Relational Database Management
- **State Management:** Provider Pattern with Service Layer Abstraction
- **API Design:** RESTful JSON API with CORS Compliance


# 🍔 Deliti - Food Delivery App

> A cool Flutter app with PHP backend for food delivery

## 🚀 Quick Start

**If you want everything to work, make sure these 5 things are ready:**

### ✅ 1. **Databases** 
- MySQL database running
- Import the SQL file included

### ✅ 2. **PHP** 
- XAMPP/WAMP running (Apache + MySQL)
- PHP files in `htdocs/deliti/api/`

### ✅ 3. **Assets**
- All images in `assets/` folder
- Menu pics, logos, icons

### ✅ 4. **Lib** 
- Flutter code in `lib/` folder
- All Dart files present

### ✅ 5. **Pubspec**
- `pubspec.yaml` configured
- Run `flutter pub get`

## 🎯 That's It!
When all 5 are ready → **Run `flutter run`** and enjoy! 🍕

---


